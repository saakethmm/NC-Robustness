...
def get_cifar10(root, cfg_trainer, train=True,
    ...
    if train:
        ...
        train_dataset = []
        npbase_dataset = np.array(base_dataset.targets)
        val_idxs = []
        # pdb.set_trace()
        for i in range (4):
            idxs = np.where(npbase_dataset == i)[0]
...
def train_val_split(base_dataset: torchvision.datasets.CIFAR10, subset_percent = 1.0):
    ...
    train_n = int(len(base_dataset) * subset_percent * 1.0 / num_classes) #1.0
...
class CIFAR10_train(torchvision.datasets.CIFAR10):
    def __init__(self, root, cfg_trainer, indexs, train=True,
        ...
        super(CIFAR10_train, self).__init__(root, train=train,
                                            ...
                                            download=download)
#         if self.train:
#             with open(self.root+'/cifar10_uniform_10/train_label.pkl', 'rb') as f: #128 to 10
#                 train_all = pickle.load(f)
#                 self.targets = train_all["label"]
#         else:
#             with open(self.root+'/cifar10_uniform_10/test_label.pkl', 'rb') as f:
...
#                 self.targets = test_all["label"]
    ...
    def symmetric_noise(self):
        ...
        changed_indices = np.random.permutation(len(self.train_data))
        for i, idx in enumerate(indices):
            if i < self.cfg_trainer['percent'] * len(self.train_data):
                self.noise_indx.append(idx)
                #self.train_labels[idx] = self.train_labels[changed_indices[i]]
...
class CIFAR10_val(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, root, cfg_trainer, indexs, train=True,
        ...
        super(CIFAR10_val, self).__init__(root, train=train,
                                          ...
                                          download=download)
#         if self.train:
#             with open(self.root+'/cifar10_uniform_10/train_label.pkl', 'rb') as f:
#                 train_all = pickle.load(f)
#                 self.targets = train_all["label"]
#         else:
#             with open(self.root+'/cifar10_uniform_10/test_label.pkl', 'rb') as f:
