...
class ConfigParser:
    ...
    def __init__(self, args, options='', timestamp=True):
        ...
        exper_name = self.path_formatter(self.config['name'])

        self.config['exper_name'] = exper_name
    ...
    def path_formatter(self, exper_name):
        # Automatically create a path name for storing checkpoints
        
        needed_types = ['data_loader','arch'] 
        needed_args = ['arch'] 


        final_exp_name = exper_name
        for setting in needed_types:
            ...
            if setting in needed_args:
                for key, val in args.items():
                    if key != "deconv_args":
                        arg_tag += '{}-{}-'.format(key, val)
                
            final_exp_name += arg_tag[:-1]
        if self.config['trainer']['sym']:
