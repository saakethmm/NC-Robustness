...
from validate_pgd_mut_l2 import validate_pgd_new
...
def main(args, config: ConfigParser):
    ...
    for step in [100]:
        ...
        model.eval()
        top1,top5,dist,distchange,dist2target,dist2targetchange,realL2,muc_dict,attack_step=validate_pgd_new(test_data_loader, model, config,fc_features)
    ...
    fig.savefig(str(args.checkpoint_path + "roubust-nearest-mutl2.pdf"), bbox_inches='tight')
    with open((args.checkpoint_path + 'info-roubust-nearest-mutl2.pkl'), 'wb') as f: 
        pickle.dump({'xlist':xlist,'acc':top1list,'acc5':top5list,'dist2center':distlist,'distchange':distchangelist,'dist2target':dist2targetlist,'dist2targetchange':dist2targetchangelist,'realL2':realL2list,'mucdict':muc_dictlist,'attackstep':attack_steplist}, f)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PyTorch Template')
    parser.add_argument('-c', '--config', type=str, default="./config_cifar100_standard.json",
                        help='config file path (default: None)')
    parser.add_argument('-s', '--checkpoint_path', type=str, default="/data1/xinshiduo/code/NCrobustness-pretrain-copy2/res_saved/models/ExperimentCIFAR100DataLoaderresnet18-num_classes-100-norm_layer_type-bn-conv_layer_type-conv-linear_layer_type-linear-activation_layer_type-relu-etf_fc-False-Seed=12/0817_053536/",
