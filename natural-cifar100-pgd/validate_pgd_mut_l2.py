...
def validate_pgd_new(test_loader, model, config,fc_features):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        ...
        # pdb.set_trace()
        # features = F.normalize(features, dim=1) torch.max(features,dim=1)
        fc_features.clear()
    ...
    for i in range(config["arch"]["args"]["num_classes"]):
        ...
        for j in range(config["arch"]["args"]["num_classes"]):
            if i==j:
                dist_temp[j]=inf
            else:
                dist_temp[j]=np.sqrt(np.sum((center[i] - center[j])**2))
        attack_target.append(0)
        attack_target[i]=np.argmin(dist_temp)
#################################################################
    for i, (input, target) in enumerate(test_loader):
        ...
        H2mean=torch.zeros([100, 512]).requires_grad_(False)    
        for idn in range(100):
            H2mean[idn,:]= torch.Tensor(center[attack_target[target[idn]]])
        ...
        for aat in range(K):
            ...
            if distflag and aat==0:
                for a in range(features.shape[0]):
                    ...
                    dist2targetlist.append(np.sqrt(np.sum((features[a, :].detach().cpu().numpy() - center[attack_target[target[a]]])**2)))
            # pdb.set_trace()
            # center[53].dot(features[0].cpu().detach().numpy()) / np.linalg.norm(center[53]) * np.linalg.norm(features[0].cpu().detach().numpy())
            
            ascend_loss =  torch.norm(features-H2mean, p='fro')
            ...
            # print(torch.max(torch.reshape(pert,(100,-1)),dim=1)[0])
            total_pert+=pert
        ...
        with torch.no_grad():
            ...
            if distflag:
                ...
                for a in range(features.shape[0]):
                    ...
                    dist2targetchangelist.append(dist2targetlist[a] - np.sqrt(np.sum((features[a, :].detach().cpu().numpy() - center[attack_target[target[a]]])**2)))
                    realL2list.append((torch.norm(calinput[a]-orig_input[a], p='fro')/(l2eps)).detach().cpu().numpy())
    ...
    # pdb.set_trace()
    return float(top1.avg),float(top5.avg),distlist,distchangelist,dist2targetlist,dist2targetchangelist,realL2list,before_class_dict,attack_step
