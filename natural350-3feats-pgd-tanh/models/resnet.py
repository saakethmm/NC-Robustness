...
#         self.linear2 = self.linear_layer(360, num_classes)
        self.linear = self.linear_layer(3, num_classes) #, bias=False
        self.linear0 = self.linear_layer(512*block.expansion, 3)
    ...
    def forward(self, x):
        ...
        out = self.activate(self.linear0(features)) # added #3
        logits = self.linear(out) # class num
