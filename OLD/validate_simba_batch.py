...
def main(args, config: ConfigParser):
    ...
    for step in range(1,args.epoch+1,args.cycle):
        checkpoint = torch.load(args.checkpoint_path+"model_epoch_"+str(step)+".pth")
        load_from_state_dict(model, checkpoint["state_dict"])
        model.eval()
        xlist.append(step)
        testset = MiniImagenet(root=config['data_loader']['args']['data_dir'], mode='test',indexs=None,cfg_trainer=config['trainer'], transform=simba_utils.MINI_TRANSFORM)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_dir', type=str, default='./res_saved/save_mini', help='directory for saving results')
    parser.add_argument('--sampled_image_dir', type=str, default='./res_saved/save_mini', help='directory to cache sampled images')
    ...
    parser.add_argument('-c', '--config', type=str, default="./config_robust_train_miniimagenet.json",
                        help='config file path (default: None)')
    parser.add_argument('-s', '--checkpoint_path', type=str, default="/data/xinshiduo/new_code/Mini/res_saved/models/Robust_experimentMiniImageNetDataLoaderresnet18-num_classes-100-norm_layer_type-bn-conv_layer_type-conv-linear_layer_type-linear-activation_layer_type-relu-etf_fc-False-Seed=8/0605_040240/",
